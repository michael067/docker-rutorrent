...
RUN apt-get update && apt-get install -y git && \
    ...
    apt-get update && \
    DEBIAN_FRONTEND=noninteractive apt-get install -y vim net-tools rtorrent unzip zip unrar rar tar mediainfo curl php-fpm php-cli php-geoip php-mbstring php-zip nginx wget ffmpeg supervisor php-xml libarchive-zip-perl libjson-perl libxml-libxml-perl irssi sox python3 python3-pip && \
